...
export default function Usuarios() {
  ...
  const load = async () => {
    ...
    catch { setErrorGlobal('Error al cargar usuarios') }
  }
...
const handleOpen = (u = null) => {
  ...
  const validar = () => {
    ...
    else if (form.usuario.trim().length < 3) err.usuario = 'Mínimo 3 caracteres'
    else if (/\s/.test(form.usuario.trim())) err.usuario = 'No se permiten espacios'
...
const handleSave = async () => {
  ...
  const handleDelete = async (id) => {
    ...
    try { await UsuarioService.delete(id); setMsg('Usuario eliminado'); load() }
              ...
              {usuarios.map((u) => (
                <TableRow key={u.id} hover>
                  ...
                  <TableCell align="center">
                    <IconButton size="small" onClick={() => handleOpen(u)}><Edit fontSize="small" /></IconButton>
                    <IconButton size="small" color="error" onClick={() => handleDelete(u.id)}disabled={u.id === user?.id} ><Delete fontSize="small" /></IconButton>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ pt: '16px !important' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField
              label="Nombre Completo *" name="nombre" value={form.nombre} onChange={handleChange}
              error={!!errores.nombre} helperText={errores.nombre}
            />
            <TextField
              label="Usuario *" name="usuario" value={form.usuario} onChange={handleChange}
              error={!!errores.usuario} helperText={errores.usuario}
            />
            <TextField
              label={editId ? 'Nueva Contraseña (dejar vacío = no cambiar)' : 'Contraseña *'}
              name="password" type="password" value={form.password} onChange={handleChange}
              error={!!errores.password} helperText={errores.password}
            />
            <TextField select label="Rol" name="rol" value={form.rol} onChange={handleChange}>
